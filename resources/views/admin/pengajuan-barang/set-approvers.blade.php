    ...
    <div>
        <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <div>
                ...
                <p class="text-sm text-zinc-400 mt-1">Atur Approver 1 dan Approver 2 untuk setiap karyawan dalam pengajuan barang.</p>
            </div>
        </div>
                        ...
                        <tbody class="divide-y divide-zinc-700">
                            @forelse ($employees as $employee)
                                <tr class="hover:bg-zinc-700/50 approver-row">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <select name="approver_barang_1[{{ $employee->id }}]" class="w-full max-w-xs p-2 bg-zinc-700 border border-zinc-600 rounded-lg text-sm text-white focus:ring-amber-500 focus:border-amber-500 approver-select">
                                            <option value="">-- Tidak Ada --</option>
                                            @foreach ($approvers as $approver)
                                                <option value="{{ $approver->id }}" @selected($employee->approver_barang_1_id == $approver->id)>
                                                    {{ $approver->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <select name="approver_barang_2[{{ $employee->id }}]" class="w-full max-w-xs p-2 bg-zinc-700 border border-zinc-600 rounded-lg text-sm text-white focus:ring-amber-500 focus:border-amber-500 approver-select">
                                            <option value="">-- Tidak Ada --</option>
                                            @foreach ($approvers as $approver)
                                                <option value="{{ $approver->id }}" @selected($employee->approver_barang_2_id == $approver->id)>
                                                    {{ $approver->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <select name="approver_barang_3[{{ $employee->id }}]" class="w-full max-w-xs p-2 bg-zinc-700 border border-zinc-600 rounded-lg text-sm text-white focus:ring-amber-500 focus:border-amber-500 approver-select">
                                            <option value="">-- Tidak Ada --</option>
                                            @foreach ($approvers as $approver)
                                                <option value="{{ $approver->id }}" @selected($employee->approver_barang_3_id == $approver->id)>
                                                    {{ $approver->name }}
                                                </option>
                                            @endforeach
                                        </select>
