...
@if ($isWeekend)
    <div class="bg-yellow-800 text-yellow-100 p-4 rounded-md mb-4 mt-6 text-center" role="alert">
        <p class="font-bold text-lg">Hari Libur Akhir Pekan</p>
        <p>Tidak ada data absensi untuk hari ini karena merupakan hari libur.</p>
    </div>
@else
            ...
            <tbody class="divide-y divide-zinc-700">
                @forelse ($absensi_harian as $record)
                    ...
                    <tr class="hover:bg-zinc-700/30">
                        ...
                        <td class="px-4 py-3">
                            @if ($record->jam_masuk)
                                <span class="font-semibold text-white">{{ \Carbon\Carbon::parse($record->jam_masuk)->format('H:i') }}</span>
                            @else
                            ...
                            @if (isset($record->jam_keluar) && $record->jam_keluar)
                                ...
                                <span class="font-semibold text-white">{{ \Carbon\Carbon::parse($record->jam_keluar)->format('H:i') }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3">
